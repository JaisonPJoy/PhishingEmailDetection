...
def Registration(request):
    ...
    if(request.POST):
        ...
        if(password==cpswd):
            ss="select count(*) from registernow  where email='"+str(email)+"'"
            c.execute(ss)
            data1=c.fetchone()
            if(data1[0]==0):
...
########################################################################################################################################
        ...
        return HttpResponseRedirect("/filecontent?file_id="+str(fileid[0]))

    return render(request,"file_sharing.html",{"data":msg,"uname":email})
